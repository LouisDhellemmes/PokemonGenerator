...
namespace PokeMonGénérator.Test
{
    ...
    { 
        ...
        {
            string talent = _talentGeneratorService.GetById(id);
            Assert.IsNotNull(talent);
            Assert.AreEqual(testList[id], talent);
        }
        ...
        {
            string talent = _talentGeneratorService.GetRandom();
            Assert.IsNotNull(talent);
            Assert.IsTrue(testList.Contains(talent));
        }
    }
}
